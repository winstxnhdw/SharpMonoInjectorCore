...
public class Memory : IDisposable {
    ...
    public byte[] ReadBytes(IntPtr address, int size) {
        ...
        if (!Native.ReadProcessMemory(this._handle, address, bytes, size)) {
    ...
    public IntPtr AllocateAndWrite(byte[] data) {
        IntPtr addr = this.Allocate(data.Length);
    ...
    public void Write(IntPtr addr, byte[] data) {
        if (Native.WriteProcessMemory(this._handle, addr, data, data.Length)) return;
